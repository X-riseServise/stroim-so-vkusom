...
Чтобы создать выпуск, войдите в `/admin/`, откройте `+ Добавить выпуск`, заполните обязательные поля и нажмите `Сохранить`. VK embed и публикация выпусков на публичном сайте пока не подключены.
...
`latest-episode.php` и `episode.php` возвращают связанные блюда. Ингредиенты преобразуются из textarea в массив строк, пустые строки удаляются. Публичный frontend загружает `/api/latest-episode.php` и `/api/episodes.php` через Vanilla JS, не вставляет пользовательский текст через `innerHTML` и открывает VK Video только после клика по preview/CTA.
